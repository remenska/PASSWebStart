...
//			modifiedKey.append("_mon");
		...
		if (humanReadable){
			LOGGER.finest("proc " + monProc
					...
					+ sumOverAllActions + ";");
			if(finalResult.indexOf("proc " + monProc+ prepend_UniversalQuantifierStateFrm + " = ")==-1)		
			...
			LOGGER.finest("proc " + monitorProcesses.get(monProc)
					+ prepend_UniversalQuantifierStateFrm + " = "
					+ sumOverAllActions + ";");
			if(finalResult.indexOf("proc " + "Mon_" + monitorProcesses.get(monProc)+ prepend_UniversalQuantifierStateFrm + " = ")==-1)
...
//		}
	...
	public String visitFalseStateFrm(
		...
		if (humanReadable){
			...
			if(finalResult.indexOf("proc " + monProc + prepend_UniversalQuantifierStateFrm + " = error;")==-1)			
				finalResult.append("proc " + monProc
						+ prepend_UniversalQuantifierStateFrm + " = error;\n\n");
		}
		else{
			...
			if(finalResult.indexOf("proc " + "Mon_" + monitorProcesses.get(monProc) + prepend_UniversalQuantifierStateFrm + " = error;")==-1)
				finalResult.append("proc " + "Mon_" + monitorProcesses.get(monProc)
						+ prepend_UniversalQuantifierStateFrm + " = error;\n\n");
		}
	...
	public String visitUniversalQuantifierStateFrm(
		...
		if (humanReadable){
			LOGGER.finest("proc " + monProc + " = sum "
					...
					+ ";");
			if(finalResult.indexOf("proc " + monProc + " = sum ")==-1)			
			...
			if(finalResult.indexOf("proc " + "Mon_" + monitorProcesses.get(monProc) + " = sum ")==-1)
				finalResult.append("proc " + "Mon_" + monitorProcesses.get(monProc)
						+ " = sum " + ctx.varsDeclList().getText() + ". "
						+ "Mon_" + monitorProcesses.get(monProc2) + varList + ";\n\n");
		}
